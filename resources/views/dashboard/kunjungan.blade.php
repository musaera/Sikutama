                ...
                <div class="mt-4 flex md:mt-0 md:ml-4">
                    <div x-data="{ open: false }" class="relative inline-block text-left">
                        ...
                        <div x-show="open" x-cloak @click.away="open = false"
                            class="absolute z-10 right-0 mt-2 w-36 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none"
                            role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                            <div class="py-1" role="none">
                                <a href="#" @click.prevent="$refs.exportFormCSV.submit()"
                                    class="text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100" role="menuitem">Export
                                    CSV</a>
                                <a href="" @click.prevent="$refs.exportFormPDF.submit()"
                                    class="text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100" role="menuitem">Export
                                    PDF</a>
                            </div>
                        </div>

                        <!-- Hidden forms to submit with current filters -->
                        <form x-ref="exportFormCSV" method="GET" action="{{ route('dashboard.export') }}">
                            <input type="hidden" name="format" value="csv">
                            <input type="hidden" name="filter" value="{{ request('filter') }}">
                            <input type="hidden" name="search" value="{{ request('search') }}">
                        </form>

                        <form x-ref="exportFormPDF" method="GET" action="{{ route('dashboard.export') }}">
                    ...
                    {{-- <a href="{{ route('dashboard.export', ['format' => 'csv', 'filter' => 'today']) }}?format=csv" class="ml-3 inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
                    ...
                    <a href="{{ route('kunjungan.create') }}"
                        class="ml-3 inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
                        ...
                        <div class="flex items-end">
                            ...
                            @if (request('search') || request('filter'))
                                <a href="{{ route('dashboard.kunjungan') }}"
                                    class="ml-3 inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
                                    Reset
                                </a>
                            @endif
                        </div>
                    ...
                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                                    ...
                                    @forelse ($kunjungan as $k)
                                        <tr>
                                            ...
                                            <div class="flex-shrink-0 h-10 w-10">
                                                @if ($k->foto_tamu)
                                                <img class="h-10 w-10 rounded-full object-cover" src="{{ asset('storage/public/uploads/' . $k->foto_tamu) }}" alt="{{ $k->nama_tamu }}">
                                                @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('kunjungan.show', $k) }}"
                                class="text-emerald-600 hover:text-emerald-900 mr-3">Detail</a>
                            <a href="{{ route('kunjungan.edit', $k) }}"
                                class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                            @if (!$k->waktu_keluar)
                                <form action="{{ route('kunjungan.keluar', $k) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="text-blue-600 hover:text-blue-900 mr-3">Catat
                                        Keluar</button>
                                </form>
                            @endif
                            <form action="{{ route('kunjungan.destroy', $k) }}" method="POST" class="inline"
